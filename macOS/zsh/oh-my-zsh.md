...
- GNOME Terminal (the default macOS terminal): Open Terminal → Preferences and click on the selected profile under Profiles. Check Custom font under Text Appearance and select `MesloLGS NF Regular`.
...
ln -sf ~/packages/config/macOS/zsh/.p10k.zsh ~/.p10k.zsh
```
...
```sh
git clone git@github.com:woomiz/config.git ~/packages/config
ln -sf ~/packages/config/macOS/zsh/.zshrc ~/.zshrc
```
